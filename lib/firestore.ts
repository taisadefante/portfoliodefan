...
function normalizeProject(id: string, data: Record<string, unknown>): Project {
  ...
  return {
    ...
    integrations: Array.isArray(data.integrations)
      ...
      : [],
    indicatedBusinesses: Array.isArray(data.indicatedBusinesses)
      ? data.indicatedBusinesses.map(String)
      : [],
    basicFlow: Array.isArray(data.basicFlow) ? data.basicFlow.map(String) : [],
  ...
  snap.docs.forEach((item) => {
    ...
    if (key in options && Array.isArray(data.values)) {
      options[key] = Array.from(
        new Set([...options[key], ...data.values]),
      ).sort();
    }
  });
...
export async function saveProjectOptions(
  ...
  await setDoc(doc(db, "projectOptions", category), { values: cleanValues });
}
